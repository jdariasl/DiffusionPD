...
class Pataka_Dataset(Dataset):
    ...
    def __init__(self, 
        ...
        self.signals, self.y_label, self.subject_group, self.db_group = self.process_select_signals(SAMPLE_RATE)
    ...
    def __getitem__(self, index):
        ...
        return torch.tensor(spec, dtype=torch.float32), torch.tensor(self.y_label[index], dtype=torch.float32), torch.tensor(self.subject_group[index], dtype=torch.float32), self.db_group[index]
    ...
    def read_data(self):
        ...
        for DB in self.DBs:
            if DB == 'Gita':
                ...
                Nspeakers = len(speaker_ids)
                paths_SaarB, labels_SaarB, speaker_ids_SaarB = Read_Saarbruecken_DB(DATA_PATH_SaarB)
    ...
    def process_select_signals(self, SAMPLE_RATE):
        ...
        overloap=2
        signals, y_label, subject_group, db_group =[],[],[],[]
        signals_sb, y_label_sb, subject_group_sb, db_group_sb =[],[],[],[]
        ...
        if len(signals_sb)>0:
            signals_sb = np.stack(signals_sb,axis=0)
            ...
            signals = np.concatenate((signals,signals_sb),axis=0)
            y_label = np.concatenate((y_label,y_label_sb),axis=0)
            subject_group = np.concatenate((subject_group,subject_group_sb),axis=0)
            db_group = np.concatenate((db_group,db_group_sb
                                       ),axis=0)

        return signals, y_label, subject_group, db_group
