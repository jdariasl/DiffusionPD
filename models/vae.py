...
class VAE(nn.Module):
    def __init__(
        ...
        self.spec_dec = torch.nn.Sequential(
            self.dec_conv1, nn.ReLU(), self.dec_conv2, nn.ReLU(), self.dec_conv3
        )
...
class VAE_DA(nn.Module):
    def __init__(
        ...
        self.fc_da = nn.Linear(self.latent_dim, 3)
    ...
    def db_classifier(self, x):
        x = ReverseLayerF.apply(x, 1.0)  # Reverse the gradient
        x = self.fc_da(x)
        return x
