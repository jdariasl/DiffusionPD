...
class UNet(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, num_classes=4, init_features=32, time_emb_dim=128):
        super(UNet, self).__init__()
        
        self.num_classes = num_classes
        self.time_emb_dim = time_emb_dim
        features = init_features
        self.encoder1 = UNetBlock(in_channels + 1, features, time_emb_dim)
        self.pool1 = nn.MaxPool1d(kernel_size=2, stride=2)
        self.encoder2 = UNetBlock(features, features * 2, time_emb_dim)
        self.pool2 = nn.MaxPool1d(kernel_size=2, stride=2)
        self.encoder3 = UNetBlock(features * 2, features * 4, time_emb_dim)
        self.pool3 = nn.MaxPool1d(kernel_size=2, stride=2)
        self.encoder4 = UNetBlock(features * 4, features * 8, time_emb_dim)
        self.pool4 = nn.MaxPool1d(kernel_size=2, stride=2)
        
        self.bottleneck = UNetBlock(features * 8, features * 16, time_emb_dim)
        
        self.upconv4 = nn.ConvTranspose1d(features * 16, features * 8, kernel_size=2, stride=2)
        self.decoder4 = UNetBlock(features * 16, features * 8, time_emb_dim)
        self.upconv3 = nn.ConvTranspose1d(features * 8, features * 4, kernel_size=2, stride=2)
        self.decoder3 = UNetBlock(features * 8, features * 4, time_emb_dim)
        self.upconv2 = nn.ConvTranspose1d(features * 4, features * 2, kernel_size=2, stride=2)
        self.decoder2 = UNetBlock(features * 4, features * 2, time_emb_dim)
        self.upconv1 = nn.ConvTranspose1d(features * 2, features, kernel_size=2, stride=2)
        self.decoder1 = UNetBlock(features * 2, features, time_emb_dim)
        ...
        self.class_mlp = nn.Sequential(
            nn.Linear(num_classes, init_features),
            nn.ReLU()
        )
    ...
    def encode(self, x, class_label, time_step):
        ...
        enc2 = self.encoder2(self.pool1(enc1), time_emb)
        enc3 = self.encoder3(self.pool2(enc2), time_emb)
    ...
    def forward(self, x, class_label, time_step):
        ...
        dec4 = self.decoder4(dec4, time_emb)
        dec3 = self.upconv3(dec4)
        dec3 = torch.cat((dec3, enc3), dim=1)
        dec3 = self.decoder3(dec3, time_emb)
        dec2 = self.upconv2(dec3)
        dec2 = torch.cat((dec2, enc2), dim=1)
        dec2 = self.decoder2(dec2, time_emb)
        dec1 = self.upconv1(dec2)
        dec1 = torch.cat((dec1, enc1), dim=1)
        dec1 = self.decoder1(dec1, time_emb)
